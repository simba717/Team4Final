...
<script>
	window.onload = function() {
		...
		xhr.open("GET", "<c:url value='/getAllOrdersByMember.json' />", true);
		xhr.send();
			...
			if (xhr.readyState == 4 && xhr.status == 200) {
				var content = "<table id='idtable' class='table table-bordered table-sm'>";
					content += "<thead><tr style='background-color: lightblue'><th>訂單號碼</th><th>總金額</th><th>會員</th>" 
							 + "<th>訂購日期</th><th>訂購狀態</th><th>明細</th></tr></thead>";
...
function query() {
	...
	/*訂單號碼、訂購時間*/
	if(char >= 0 && char <= 9) {		
		xhr1.open("Get", "<c:url value='/orders/memberSearchOrder=' />" + search , true);
		xhr1.send();
	} else {
